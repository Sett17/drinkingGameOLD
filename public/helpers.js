function isMobile() {       // check if device is a mobile device
    let hasTouchScreen = false
    if ("maxTouchPoints" in navigator) {    // check for touch, probably breaks down with other touch stuff except phones
        hasTouchScreen = navigator.maxTouchPoints > 0
    } else if ("msMaxTouchPoints" in navigator) {   // same as above
        hasTouchScreen = navigator.msMaxTouchPoints > 0
    } else {
        var mQ = window.matchMedia && matchMedia("(pointer:coarse)")
        if (mQ && mQ.media === "(pointer:coarse)") {    // I don't really know
            hasTouchScreen = !!mQ.matches
        } else if ('orientation' in window) { // deprecated, but good last measure
            hasTouchScreen = true
        }
    }
...
class Session extends Map { // 'borrowed' code for saving objects in session vars
    set(id, value) {    // translates them to json
        if (typeof value === 'object') value = JSON.stringify(value)
        sessionStorage.setItem(id, value)
    }

    get(id) {           // translates them from json
...
function coerce(num, min, max) {    // coerces number between min and max
    if (num < min) {
        return min
    } else if (num > max) {
        return max
...
// card dragging stuff
let oriX = 0, oriY = 0, dX = 0, dY = 40, offsetX = 0, offsetY = 40, angle = 0   // y offs of 40 to make the composition better
let isMoving = false
function handleDragStart(e) {   // dragging started, reset everything
...
function handleDragEnd(e) {     // do when finger is released
    ...
    if (dX < zoneSize[0] &&
        dX > -zoneSize[0] &&
        dY < zoneSize[1]) { // do when card is NOT thrown out
        ...
        setTimeout(() => { // reset/animate everything else
    ...
    } else { // card being thrown out
        if (dY > zoneSize[1] &&
            dX < zoneSize[0] &&
            dX > -zoneSize[0]) { // throw out bottom
            isMoving = true
        ...
        if (gameRunning.get()) { // next card
            setTimeout(() => {
                newCard()
                isMoving = false
            }, animTimeOut * 1.5)
        } else {          // last card thrown out, back to startmenu
            setTimeout(() => {
...
let turnTime = 5
let turn = true
function handleDragMove(e) {    // do when dragging
    if (!isMoving) {    // don't do anything while card is animated
        if (turn) {     // only do if turnTime milliseconds have past since last (close to every other)
            turn = false
            setTimeout(() => { turn = true }, turnTime) // enable turn after turnTime !IMPORTANT
            lastCoord = [e.touches[0].clientX, e.touches[0].clientY]
            dX = e.touches[0].clientX - oriX
            dY = e.touches[0].clientY - oriY
            if (dX >= 0) { // concerning left and right 'cause coerce doesn't do what it should with negatives or I just fucked up
                angle = ((coerce(dX, 0, 200) - 0) / (200 - 0) * (20 - 0) + 0) * Math.PI / 180 // mapping number range to another; 0:200->0:20
                document.querySelector('#play-name').style.opacity = ((coerce(dX, 0, 100) - 0) / (100 - 0) * (0.0 - 1.0) + 1.0) // 0:100->0.0:1.0
            } else {
                angle = -((coerce(-dX, 0, 200) - 0) / (200 - 0) * (20 - 0) + 0) * Math.PI / 180 // dX and angle are negated; 0:200->0:20
                document.querySelector('#play-name').style.opacity = ((coerce(-dX, 0, 100) - 0) / (100 - 0) * (0.0 - 1.0) + 1.0) // dX is negated; 0:100->0.0:1.0
            }
...
function addDrags(el) {     // add dragging functionality to element
...
function getMatrix(angle, tx, ty) {     //makes 2d transformation matrix from angle and translation
    return `matrix(${Math.cos(angle)}, ${Math.sin(angle)}, ${-Math.sin(angle)}, ${Math.cos(angle)}, ${tx}, ${ty})`
}

function loadCards() {      // loading the cards, duh; used in init
    return fetch('./assets/cards.json')
        .then(d => d.json())
} // returns promise

String.prototype.hash = function () {   // 'borrowed', not used rn
    var hash = 0
...
Array.prototype.removeElement = function (obj) {   // remove object out of array, because that is not buil-in to js, smh
...
function reloadFresh() {    // unregister service-worker, and then reload the page
...
function shareRoutine() {   // share function on startmenu
    try {                   // native share function on phone/browser (unfortunately asks browser to share...)
        navigator.share({
            url: "https://drinkingapp-4376b.web.app/",
            title: "Trink! Trink!",
        })
            .then(d => console.log(d))
    } catch (e) {           // if share function not there, put it into clipboard and tell user
        console.log("not on a smartphone, copying to clipbaord instead of sharing")
        try {
            navigator.clipboard.writeText("https://drinkingapp-4376b.web.app/")
                .then(d => {
                    alert("In die Zwischenablage kopiert")  // prolly should make a custom modal, well
                })
        } catch (e) {   // don't know why that shouldn't work, but you do that with navigator when not doing feature detec
            console.log(e)
        }
    }
}

const gameRunning = {   // obj for the game state
    get: () => { return sess.get('gameRunning') },
    turnOn: () => {
...
const availCards = {       // obj to keep track of the deck
    deck: [],
    get: () => {
    ...
    update: (par) => {
        if (!gameRunning.get()) {   // only update deckif game is not running
            this.deck = par
    ...
    remove: (el) => {   // removes single card from deck
