const options = {
    updateViews: () => { options.updateDifficultyView() }, difficultyLvl: undefined, difficultyChanged: (arg) => {
        ...
        options.difficultyLvl = options.difficultyLvl || parseInt(sess.get('difficultyLvl')) || 0
        return options.difficultyLvl
    }, updateDifficultyView: () => {
        let z = options.getDifficulty()
        switch (z) {
            case 0: document.querySelector('#options-difficultyWrapper .options-chkbx[data-id="0"]').setAttribute('active', '')
